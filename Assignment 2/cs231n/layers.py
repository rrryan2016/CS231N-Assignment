...
def affine_forward(x, w, b):
  ...
  # M - the number of classifications
  # x.size - directly know how many items are there in the multi-dimension array,x
  pass
  N, D = x.shape[0], x.size // x.shape[0] # point different in py2 and py3
...
def affine_backward(dout, cache):
  ...
  # Out = W*x + b
  # Multiply Principle
  # dout/dw = dout/dw/dx * dx
  # dout/dx = dout/dw/dx * dw
  # Add principle
...
def relu_backward(dout, cache):
  ...
  #############################################################################
  pass
  dx = dout * (x > 0) # if x<0, set it to be 0
  # the dicinple of max() hinge function
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    out = gamma * x_hat + beta # according to the BN Math formula
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  x_padded = np.pad(x, ((0,), (0,), (pad,), (pad,)), 'constant') # pad alongside four dimensions / do paddling in both sides
  N, C, H, W = x.shape
  F, C, HH, WW = w.shape
  output_height = 1 + (H + 2 * pad - HH) // stride # Alter Point
  output_width = 1 + (W + 2 * pad - WW) // stride # Alter Point
  ...
  for i in range(output_height):
      for j in range(output_width):
          ...
          for k in range(F):
              out[:, k, i, j] = np.sum(x_padded_mask * w[k, :, :, :], axis=(1,2,3)) # conv operation here
...
def conv_backward_naive(dout, cache):
  ...
  # x_pad = np.pad(x, ((0,), (0,), (pad,), (pad,)), mode='constant', constant_values=0)
  # # I guess the code line above is unnecessary, so I comment it.
...
def max_pool_forward_naive(x, pool_param):
  ...
  out_height = H // pool_height # Alter Point
  out_width = W // pool_width # Alter Point
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(out_height):
      for j in range(out_width):
          ...
          x_mask = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] # Why dx will change after this operation
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  pass
  N, C, H, W = x.shape
  x = x.transpose(0, 2, 3, 1).reshape(N * H * W, C) # This logic must be paid attention to !!!
  out, cache = batchnorm_forward(x, gamma, beta, bn_param)
  out = out.reshape(N, H, W, C).transpose(0, 3, 1, 2) #Bravo! Based on the former batchnorm_forward. The transfer and application are based on reshape() and transpose()
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def svm_loss(x, y):
  ...
  # print("np.arange(N) \n ", a.shape, "\n")
  # print("x \n ", x.shape, "\n")
  # print("x[np.arange(N), y] \n", x[a, y].shape, "\n")

  margins = np.maximum(0, x - correct_class_scores[:, np.newaxis] + 1.0) # SVM Loss Function, delta = 1
...
def softmax_loss(x, y):
  ...
  N = x.shape[0]
  loss = -np.sum(np.log(probs[np.arange(N), y])) / N # Softmax Loss Function
  dx = probs.copy()
  dx[np.arange(N), y] -= 1
  dx /= N
  return loss, dx
