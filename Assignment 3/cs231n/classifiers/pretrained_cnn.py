...
class PretrainedCNN(object):
  def __init__(self, dtype=np.float32, num_classes=100, input_size=64, h5_file=None):
    ...
    fan_in = (cur_size * cur_size * self.num_filters[-1]) # Alter Point
    # fan_in = cur_size * cur_size * self.num_filters[-1]

    # print(fan_in)
    # print(type(fan_in))

    # print (np.random.randn(fan_in, hidden_dim))
    # print(type(np.sqrt(np.random.randn(fan_in, hidden_dim))))

    self.params['W%d' % (i + 2)] = np.sqrt(2.0 // int(fan_in)) * np.random.randn(int(fan_in), hidden_dim)# Alter Point
